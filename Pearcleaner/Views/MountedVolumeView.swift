...
//                }
    ...
    private func ejectVolume(_ volume: VolumeInfo) {
        let currentIndex = appState.volumeInfos.firstIndex(where: { $0.id == volume.id })

        DispatchQueue.global(qos: .userInitiated).async {
            let success = NSWorkspace.shared.unmountAndEjectDevice(atPath: volume.path)
